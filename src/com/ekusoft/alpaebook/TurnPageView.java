...
public class TurnPageView extends View {
	 ...
	 * 当前页的索引监听
	 */
	private OnTurnPageIndexListener listener;
	...
	public boolean onTouchEvent(MotionEvent event) {
		/*if (index < 1 || index > mBitmapLists.size() -2) {
			return true;
		}*/
		float x = event.getX();
		float y = event.getY();
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if(canDragOver()){
				startAnimation(ANIM_DURATION ,event.getX());
			}else {
			...
			if (canDragOver()) {
				if (listener != null) {
					listener.setCurPageIndex(index + 1);
				}
			}
			break;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isTurnLeft) {
			if (index == 1) {
				drawCurrPage(canvas, mBitmapLists.get(0));
			    drawNextPageAndShadow(canvas, mBitmapLists.get(1));
				drawCurrPageShadow(canvas);
				drawCurrBackPage(canvas, mBitmapLists.get(0));
			}else if (index == 0) {
				drawCurrPage(canvas, mBitmapLists.get(0));
			    drawNextPageAndShadow(canvas, mBitmapLists.get(0));
				drawCurrPageShadow(canvas);
				drawCurrBackPage(canvas, mBitmapLists.get(0));
			}else {
				drawCurrPage(canvas, mBitmapLists.get(index - 1));
				drawNextPageAndShadow(canvas, mBitmapLists.get(index));
				drawCurrPageShadow(canvas);
				drawCurrBackPage(canvas, mBitmapLists.get(index - 1));
			}
	 ...
	 * 开始翻转动画
	 */
	private void startAnimation(int duration ,float x) {
		int dx, dy;
	...
	private boolean isLeftPos(float x) {
		DisplayMetrics metric = new DisplayMetrics();
        ((TurnPageActivity)getContext()).getWindowManager().getDefaultDisplay().getMetrics(metric);
	...
	public void setBitmaps(Bitmap currPage) {
		mBitmapLists.add(currPage);
	}
	
	interface OnTurnPageIndexListener {
		public void setCurPageIndex(int index);
	}
	
	public void addTurnPageIndexChanged(OnTurnPageIndexListener listener) {
